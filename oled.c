 ...
 *
 * @note
 * 为保证中文显示正常 请将编译器的字符集设置为U// 将 main.c:103, 113-117 改为：
char message[50];
struct tm* now;
...
while (1)
{
    ...
    OLED_NewFrame();
    // 用 afont12x6 小字体，宽度 6px × 19字符 = 114px < 128px
    OLED_PrintASCIIString(0, 0, message, &afont12x6, OLED_COLOR_NORMAL);
    OLED_ShowFrame();
    HAL_Delay(100);
}// 将 main.c:103, 113-117 改为：
char message[50];
struct tm* now;
 ...
 * @param w 区域宽度
 * @param h 区域高度
 * @note 此函数将显存中从(x,y)开始的w*h个像素取反(0变1, 1变0), 超出屏幕的部分会被裁剪
 * @note 重复调用同一区域会恢复原状(取反是可逆的)
 */
void OLED_InvertArea(uint8_t x, uint8_t y, uint8_t w, uint8_t h)
{
  ...
  // 计算实际生效的区域(裁剪到屏幕范围内), 用uint16_t避免x+w溢出
  ...
  if (yEnd > OLED_ROW)
    yEnd = OLED_ROW;
  if (x >= OLED_COLUMN || y >= OLED_ROW || xEnd <= x || yEnd <= y)
    return;

  uint8_t startPage = y / 8;               // 起始页
  uint8_t startBit = y % 8;                // 起始页内起始位
  uint8_t endPage = (uint8_t)((yEnd - 1) / 8); // 结束页
  uint8_t endBit = (uint8_t)((yEnd - 1) % 8);  // 结束页内结束位

  for (uint8_t col = x; col < xEnd; col++)
  {
    if (startPage == endPage)
    {
      // 区域在同一页内, 仅取反部分位
      uint8_t mask = (0xff >> (7 - endBit)) & (0xff << startBit);
      OLED_GRAM[startPage][col] ^= mask;
    }
    else
    {
      OLED_GRAM[startPage][col] ^= (0xff << startBit); // 顶部不完整字节
      for (uint8_t page = startPage + 1; page < endPage; page++)
      {
        OLED_GRAM[page][col] ^= 0xff; // 中间完整字节
      }
      OLED_GRAM[endPage][col] ^= (0xff >> (7 - endBit)); // 底部不完整字节
    }
  }
}
